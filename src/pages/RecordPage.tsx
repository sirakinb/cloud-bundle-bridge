...
const RecordPage = () => {
  ...
  const handleSaveRecording = async (folderId: string) => {
    ...
    try {
      // Process the audio to ensure compatibility
      const { audioUrl, format, processedBlob } = await processAudioForCompatibility(finalAudioBlob);
      setAudioFormat(format);
      
      // Create a mock transcription (in a real app, this would come from Deepgram)
      const mockTranscription = "This is a sample transcription. In a real application, this would be generated by the Deepgram API based on the audio content.";
      ...
      console.log("Saving recording with format:", format);
      ...
      toast({
        ...
        description: `Your recording "${recordingTitle}" has been saved as ${format.toUpperCase()}.`,
      });
      ...
      toast({
        ...
        description: "There was an error processing the audio. Using fallback format.",
      });
      
      // Use fallback if processing fails
      ...
      reader.onloadend = () => {
        ...
        console.log("Saving recording with fallback format");
        saveRecording(newRecording);
  ...
  const processFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { audioUrl, format, transcription } = await handleAudioFileUpload(file);
      ...
      sonnerToast("File Uploaded", {
        description: `"${file.name}" has been processed as ${format.toUpperCase()}.`,
      });
      ...
      // Automatically open the folder dialog to save the uploaded file
      setFolderDialogOpen(true);
    } catch (error) {
