...
const RecordPage = () => {
  ...
  const handleSaveRecording = async (folderId: string) => {
    ...
    try {
      ...
      const { audioUrl, metadata } = await processAudioForCompatibility(finalAudioBlob);
      ...
      toast({
        ...
        description: `Audio optimized to ${metadata.format.toUpperCase()} format${metadata.sampleRate ? ` at ${metadata.sampleRate/1000}kHz` : ''}.`,
      });
      ...
      console.log("Saving recording with format:", metadata.format);
      ...
      toast({
        ...
        description: `Your recording "${recordingTitle}" has been saved as ${metadata.format.toUpperCase()}.`,
      });
